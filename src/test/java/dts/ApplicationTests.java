	...
	public void CreateNewUser () throws Exception
	{
		...
		//Set URL for POST user
		   ...
		   //AND the server returns the JOSN (User boundary - player) //Date bigger to test if new
			...
			.isEqualTo(Application.APPLICATION_NAME);
			assertThat(actual.getRole())
			.isEqualTo(UserRole.PLAYER);
	...
	public void PutUser () throws Exception {
        ...
        // User made
		   ...
		   //THEN the result HTTP STATUS 2xx 
			...
			UserBoundary actualGET = this.restTemplate
					.getForObject(urlTest, UserBoundary.class);
			assertThat(actualGET.getUserId())
			.isNotNull();
		   //AND the server returns the JOSN (User boundary - player)(DEMO)
			assertThat(actualGET.getUserId().getEmail())
			.isEqualTo("demoTEST@maildomain.com");
			assertThat(actualGET.getUserId().getSpace())
			.isEqualTo(Application.APPLICATION_NAME);
			assertThat(actualGET.getRole())
			.isEqualTo(UserRole.PLAYER);
			assertThat(actualGET.getUsername())
			.isEqualTo("JUnit Test");
			assertThat(actualGET.getAvatar())
			.isEqualTo("0.-(-!-)");
...
////Test GET////
	...
	public void GetItem () throws Exception {
		String urlTest;
		String itemID;
		// Create new Item and test the Get as last test we don't save the ID ITEM! here we can test !
		urlTest = this.url + "/items/JunitTest/JunitTester@Gmail.com";
		...
		ItemBoundary actual = this.restTemplate
					.getForObject(urlTest, ItemBoundary.class);
		   //THEN the result HTTP STATUS 2xx 
		   //AND the server returns the JOSN (Item boundary)
			assertThat(actual.getItemId())
			.isNotNull(); // User received 
			assertThat(actual.getItemId().getId())
			.isEqualTo(itemID);
			assertThat(actual.getCreatedBy().getClass()).hasSameClassAs(UserBoundary.class);
			assertThat(actual.getLocation().getClass()).hasSameClassAs(LocationBoundary.class);
	...
	public void GetArrayOfItems () throws Exception {
		...
		ItemBoundary[] actual = this.restTemplate
		   ...
		   //AND the server returns the JOSN (Array of items)
			assertThat(actual)
			.hasSizeGreaterThan(0); // if 0 not good
...
////Test PUT	////	
	...
	public void PutItem () throws Exception {
		...
		// User already need to be existed as Create new User run's before
		//Set URL for PUT array /dts/items/{managerSpace}/{managerEmail}/{itemSpace}/{itemId}
		urlTest = this.url + "/items/" + Application.APPLICATION_NAME +"/demo@maildomain.com/" + Application.APPLICATION_NAME + "/" +itemId ;
	       //WHEN I PUT change /dts/items/{managerSpace}/{managerEmail}/{itemSpace}/{itemId}
		this.restTemplate
				.put(urlTest, changeItem);
		   //THEN the result HTTP STATUS 2xx 
		...
		urlTest = this.url + "/operations";
	       ...
	       //WHEN I POST /dts/operations
			OperationBoundary actual = this.restTemplate
					.postForObject(urlTest, new HashMap<>(), OperationBoundary.class); // new HashMap is same as {} empty body
		   //THEN the result HTTP STATUS 2xx 
		   //AND the server returns the JOSN (Operation boundary) //Date bigger to test if new
			assertThat(actual.getOperationId())
			.isNotNull();
			assertThat(actual.getCreatedTimestamp().getTime())
			.isGreaterThan(now);
			assertThat(actual.getType())
			.isEqualTo("operationType");
			assertThat(actual.getItem().getClass()).hasSameClassAs(ItemBoundary.class);
			assertThat(actual.getInvokedBy().getClass()).hasSameClassAs(UserBoundary.class);
...
////Test GET////
	...
	public void OperationGet () throws Exception {
		...
		// Create new Item and test the Get as last test we don't save the ID ITEM! here we can test !
		urlTest = this.url + "/operations";
		OperationBoundary newOpertation = this.restTemplate
				.postForObject(urlTest, new HashMap<>(), OperationBoundary.class); // new HashMap is same as {} empty body
		assertThat(newOpertation.getClass()).hasSameClassAs(OperationBoundary.class); // Check we received OperationBoundary back
		OperationBoundary newOpertation2 = this.restTemplate
				.postForObject(urlTest, new HashMap<>(), OperationBoundary.class); // new HashMap is same as {} empty body
		assertThat(newOpertation2.getClass()).hasSameClassAs(OperationBoundary.class);
			
		//Set URL for GET "/dts/operations/{adminSpace}/{adminEmail}"
		urlTest = this.url + "/operations/" + Application.APPLICATION_NAME +"/demo@maildomain.com";
	       //GIVEN the server is up (Tested)
	       //WHEN I GET  "/dts/operations/{adminSpace}/{adminEmail}"
		OperationBoundary[] actual = this.restTemplate
				.getForObject(urlTest, OperationBoundary[].class);
		   //THEN the result HTTP STATUS 2xx 
		   //AND the server returns the JOSN (Operations Array boundary)
	   //THEN the result HTTP STATUS 2xx 
	   //AND the server returns the JOSN (Array of items)
		assertThat(actual).isNotNull();
		assertThat(actual)
		.hasSizeGreaterThan(1); // if 0 not good
...
////------------- Admin API test	--------------- ////	
	...
	public void DeleteAllUsersAPI () throws Exception {
		...
		int i;
		int limit = 10;
		// GIVEN the server is not Empty and have users
	    urlTest = this.url + "/users";
        ...
        //Get all the user 
        urlTest = this.url +"/admin/users/" +Application.APPLICATION_NAME + "/demo@maildomain.com";
        
        // Get all the Users
		//UserBoundary[] actualAllUser = this.restTemplate
				//.getForObject(urlTest, UserBoundary[].class);
		//assertThat(actualAllUser)
		//.hasSizeGreaterThan(9);
		//WHEN post Delete all Users "/dts/admin/users/{adminSpace}/{adminEmail}"
		//urlTest = this.url + "/admin/users/" + Application.APPLICATION_NAME +"/demo@maildomain.com";
		//this.restTemplate.delete(urlTest);
	   //THAN get all user equals 0
			
	        //Get all the user 
	        //urlTest = this.url +"/admin/users/" + Application.APPLICATION_NAME + "/demo@maildomain.com";
	        
	        // Get all the Users
			//UserBoundary[] deletedUsers = this.restTemplate
					//.getForObject(urlTest, UserBoundary[].class);
